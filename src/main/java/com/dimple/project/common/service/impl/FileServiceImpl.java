...
public class FileServiceImpl implements FileService {
    ...
    @Override
    public String insertFastdfsImageFile(MultipartFile file) throws Exception {
        String upload = fastdfsUtil.upload(file);
        String baseDir = "http://116.62.231.166";
        FileItemInfo fileItemInfo = new FileItemInfo(file.getOriginalFilename(), String.valueOf(file.hashCode()),
                file.getSize(), file.getContentType(), new Date(), FileItemInfo.ServerType.LOCAL.getServerType(), baseDir + "/" + upload);
        fileItemInfoMapper.insertFileItem(fileItemInfo);
        return fileItemInfo.getPath();
    }
